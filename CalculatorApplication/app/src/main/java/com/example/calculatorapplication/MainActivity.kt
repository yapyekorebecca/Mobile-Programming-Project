...
class MainActivity : AppCompatActivity() {
    ...
    /*private fun calculateResult(){ try {

            var expressionString = expression.text.toString()

 

            // Replace percentage sign with "/100"

            expressionString = expressionString.replace("%", "/100")

 

            val expressionInput = ExpressionBuilder(expressionString).build()

            val expressionOutput = expressionInput.evaluate()

 

            result.text = expressionOutput.toString()
...
//            val isWholeNumber = expressionOutput == expressionOutput.toLong().toDouble()

//

//            if (isWholeNumber) {

//                result.text = expressionOutput.toLong().toString() // Convert to Long to remove decimal point
...
//            if (expressionOutput.isFinite()) {

//                if (expressionOutput == 0.0 && expressionString.contains("/0")) {
...
//                result.text = "Error: Invalid expression"
...
//            if (expressionOutput.isInfinite()) {
...
//            if (expressionOutput.isNaN() || expressionOutput.isInfinite()) {

//                // Check if the result is NaN or infinite (division by zero)
